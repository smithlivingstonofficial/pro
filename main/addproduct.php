...
<select name="code" style="width:265px; height:30px; margin-left:-5px;" required>
<option value="">Select Brand</option>
	...
	?>
		<option value="<?php echo $row['id']; ?>"><?php echo $row['note']; ?></option>
	<?php
	}
	?>
</select><br>
...
<option value="">Select Supplier</option>
	<?php
	include('../connect.php');
	$result = $db->prepare("SELECT * FROM suppliers"); // Corrected table name
	$result->execute();
	while($row = $result->fetch()){ // Changed for loop to while loop
	?>
		<option value="<?php echo $row['id']; ?>"><?php echo $row['supplier_name']; ?></option> <!-- Corrected column name -->
	<?php
	}
	?>
</select><br>
